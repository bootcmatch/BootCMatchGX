...
#endif
  sync_solution(H_halo_info[level], A, u);
#if DEBUG_SMART_AGG == 1
...
void relaxCoarsest(handles *h, int k, int level, hierarchy *hrrch, vector<vtype> *f, int relax_type, vtype relax_weight, vector<vtype> *u, vector<vtype> **u_, itype nlocal, bool forward=true){
  if(relax_type == 0){
    jacobi_adaptive_miniwarp_coarsest(h->cusparse_h0, h->cublas_h, k, hrrch->A_array[level], u, u_, f, hrrch->D_array[level], relax_weight);
  }else if(relax_type == 4){
    jacobi_adaptive_miniwarp_coarsest(h->cusparse_h0, h->cublas_h, k, hrrch->A_array[level], u, u_, f, hrrch->M_array[level], relax_weight);
  }
}
