...
int main(int argc, char** argv)
{
    ...
    if (ISMASTER) {
        fprintf(stderr, "Reading/generating input matrix\n");
    }

    CSR* dlA = NULL;
    if (opt == MTX) { // The master reads the matrix and distributes it.
        dlA = read_local_matrix_from_mtx_host(mtx_file_name);
    } else if (opt == LAP_3D) {
        dlA = generate_lap3d_local_matrix_host(generator, lap_3d_file);
    } else if (opt == LAP) {
        dlA = generate_lap_local_matrix_host(n);
    }
    ...
    CSR* dlAt = /*(nprocs == 1 && !force_multiproc) ? CSRm::Transpose_local(dlA, log_file) :*/ CSRm::transpose_nogpu(dlA, log_file);
    auto t2 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> ms_double = t2 - t1;

    if (ISMASTER) {
        std::cerr << "Operation took " << ms_double.count() << " ms.\n";
    }
