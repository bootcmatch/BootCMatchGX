...
int *getmct(int *Col, int nnz, int f, int l, int *uvs, int **bitcol, int *bitcolsize, int num_thr) {
    ...
    if ((*bitcol)!=NULL) {
        ...
        if (nnz>sizeof_buffer_4_getmct) {
            if (sizeof_buffer_4_getmct>0)
                cudaFree(buffer_4_getmct);
            sizeof_buffer_4_getmct = nnz;
            cudaMalloc_CNT
            cudaMalloc(&buffer_4_getmct,sizeof(int)*sizeof_buffer_4_getmct);
        }
        d_output = buffer_4_getmct;
        d_size=cuCompactor::compact<int>(Col,d_output,nnz,int_predicate(),num_thr,f,l);
    ...
    cudaMemcpy(h_ptr,dv_ptr,uvs[0]*sizeof(int),cudaMemcpyDeviceToHost);
...
//         cudaMalloc(bitcol, sizeof(int)*(uvs[0]));
        // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        *bitcol = CustomCudaMalloc::alloc_itype(uvs[0], 1);
        // ---------------------------------------------
        cudaMemcpy(*bitcol,dv_ptr,uvs[0]*sizeof(int),cudaMemcpyDeviceToDevice);
        *bitcolsize = *uvs;
//         cudaFree(d_output);
    }
    return h_ptr;
...
int *getmct_4shrink(int *Col, int nnz, int f, int l, bool first_or_last, int *uvs, int **bitcol, int *bitcolsize, int* post_local, int num_thr) {
    ...
    if (first_or_last == true) {
        if (f == 0) {
            ...
            thrust::sequence(result.begin() + uv.size(), result.end(), f);
            first_post_local = (unsigned int) uv.size();
        }
    }else{
        thrust::device_vector<int> locals(l - f +1, 1);
        thrust::sequence(locals.begin(), locals.end(), f);
            
        if (alloced_idx == false) {
            cudaMalloc_CNT
            cudaMalloc( (void**)&idx_4shrink, sizeof( unsigned int ) );
            alloced_idx = true;
        }
        unsigned int *dev_idx = idx_4shrink;
        
        cudaMemset(dev_idx, 0, sizeof(unsigned int));
        ...
        primo_kernel<<<gb.g, gb.b>>>(dv_local, dv_uv, dv_result, uv.size(), locals.size(), f, dev_idx);
        cudaMemcpy(&first_post_local, dev_idx, sizeof(unsigned int), cudaMemcpyDeviceToHost );
...
//         cudaFree(d_output);
    ...
    *post_local = (int) first_post_local;
    ...
    cudaMalloc_CNT
    cudaMalloc(&d_output, uvs[0] * sizeof(int));
    cudaMemcpy(d_output, dv_ptr, uvs[0] * sizeof(int),cudaMemcpyDeviceToDevice);
