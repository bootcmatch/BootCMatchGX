 ...
 */
template <typename OriginalType, typename MappedType, typename Mapper>
__global__ void _deviceMapWithIndex(OriginalType* d_src, size_t n, Mapper mapper, MappedType* d_dst)
{
    int tid = threadIdx.x + blockDim.x * blockIdx.x;
    while (tid < n) {
        d_dst[tid] = mapper(tid, d_src[tid]);
        tid += blockDim.x * gridDim.x;
    }
}
 ...
 */
template <typename OriginalType, typename MappedType, typename Mapper>
MappedType* deviceMapWithIndex(OriginalType* d_arr, size_t n, Mapper mapper)
{
    if (!n) {
        return NULL;
    }
    ...
    MappedType* d_ret = CUDA_MALLOC(MappedType, n);
    GridBlock gb = getKernelParams(n);
    _deviceMapWithIndex<<<gb.g, gb.b>>>(d_arr, n, mapper, d_ret);
    cudaError_t err = cudaDeviceSynchronize();
    CHECK_DEVICE(err);
