...
__host__ __device__ __forceinline__
    IsNotInRange<T, F>
    make_is_not_in_range(T min, T max, F f)
{
    return IsNotInRange<T, F>(min, max, f);
}
...
template <typename T, typename F>
void deviceFilter(T* d_arr, T* d_filteredArr, size_t len, F filter, size_t* filteredSize)
{
    if (!len) {
        *filteredSize = 0;
        return;
