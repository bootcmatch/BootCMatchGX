...
#define TRACE(...)                                                          \
    do {                                                                    \
        if (trace_enabled) {                                                \
            _MPI_ENV;                                                       \
            int pid = getpid();                                             \
            fprintf(stderr, "MPI[%d] PID[%d] %s:%d (in function %s) ",      \
                myid, pid, __FILE__, __LINE__, __func__);                   \
            fprintf(stderr, __VA_ARGS__);                                   \
            fprintf(stderr, "\n");                                          \
        }                                                                   \
    } while (0)

#define TRACE_PROC(mpi_nr, ...)                                             \
    do {                                                                    \
        if (myid == mpi_nr && trace_enabled) {                              \
            _MPI_ENV;                                                       \
            int pid = getpid();                                             \
            fprintf(stderr, "MPI[%d] PID[%d] %s:%d (in function %s) ",      \
                myid, pid, __FILE__, __LINE__, __func__);                   \
...
#define CHECK_DEVICE(X)                                                                       \
    {                                                                                         \
        ...
        if (status != cudaSuccess) {                                                          \
            ...
            DIE("[ERROR DEVICE] :\n\tMYID: %d, %s; LINE: %d; FILE: %s\n", myid, err_str, __LINE__, __FILE__); \
        }                                                                                     \
    }
...
#define PRINT_POINTER_INFO(ptr) printPointerInfo(stderr, ptr, #ptr, __FILE__, __LINE__)

#define ASSERT(a)                                                          \
    do {                                                                   \
        if (!(a)) {                                                        \
            DIE("Assert failed at %s:%d\n\t%s\n", __FILE__, __LINE__, #a); \
        }                                                                  \
    } while (0)
