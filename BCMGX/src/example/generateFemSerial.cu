...
int main(int argc, char** argv)
{
    ...
    CSR *A = generateLocalFem_1proc(nx, ny, P, Q, E, nu, &rhs);
...

    {
        ...
        snprintf(filename, 1024, "%s/A_%dx%d_%dx%d_serial.mtx", output_dir.c_str(), nx, ny, P, Q);
        export_matrix_market(filename, A);

        snprintf(filename, 1024, "%s/rhs_%dx%d_%dx%d_serial.mtx", output_dir.c_str(), nx, ny, P, Q);
        export_matrix_market(filename, rhs);
    }
