...
template <typename T>
__inline__
void chop_array_MPI(int nprocs, int n, int *chunks, int *chunkn){
  int i;
...
template<typename Tst = stype, typename Tgs = gstype, typename Ti = itype>
struct CSRtype{
  Tst nnz; // number of non-zero
  Tst n; // rows number
  Tgs m; // columns number
  Tst shrinked_m; // columns number for the shrinked matrix
  ...
  Ti *col; // array of the column index
  Ti *row; // array of the pointer of the first nnz element of the rows
  Ti *shrinked_col; // array of the shrinked column indexes
