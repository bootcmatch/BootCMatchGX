...
__global__ void computeBlockCounts(T* d_input,int length,int*d_BlockCounts,Predicate predicate,T f, T l){
...
template <typename T,typename Predicate>
__global__ void compactK(T* d_input,int length, T* d_output,int* d_BlocksOffset,Predicate predicate, T f, T l){
	int idx = threadIdx.x + blockIdx.x*blockDim.x;
...
template <typename T,typename Predicate>
int compact(T* d_input,T* d_output,int length, Predicate predicate, int blockSize, T f, T l){
  ...
  try{
    ...
    std::cerr << "Error inside getmct:compact:thrust::exclusive_scan    " << e.what() << std::endl;
    exit(-1);
  }
