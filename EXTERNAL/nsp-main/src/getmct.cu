...
using namespace std;
static int *buffer_4_getmct;
static int sizeof_buffer_4_getmct = 0;
static unsigned int * idx_4shrink;
static bool alloced_idx = false;
static int *glob_d_BlocksCount;
static int *glob_d_BlocksOffset;;
...
int *getmct(int *Col, int nnz, int f, int l, int *uvs, int **bitcol, int *bitcolsize, int num_thr) {
    ...
    if ((*bitcol) == NULL){
      try{
        ...
        std::cerr << "Error inside getmct:thrust::sort   " << e.what() << std::endl;
        exit(-1);
      }
    }
