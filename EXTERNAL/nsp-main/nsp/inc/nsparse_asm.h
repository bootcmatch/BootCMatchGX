...
// load int from global memory without fetching L2 cache
__device__ __inline__ int load_glob(const int *val) {
int return_value;
asm("ld.global.cv.s32 %0, [%1];" : "=r"(return_value) : "l"(val));
return return_value;
}
 
// load long int from global memory without fetching L2 cache
__device__ __inline__ long int load_glob(const long int *val) {
long int return_value;
asm("ld.global.cv.s64 %0, [%1];" : "=l"(return_value) : "l"(val));
return return_value;
}
 
// load float from global memory without fetching L2 cache
__device__ __inline__ float load_glob(const float *val) {
float return_value;
asm("ld.global.cv.f32 %0, [%1];" : "=f"(return_value) : "l"(val));
return return_value;
}
 
// load double from global memory without fetching L2 cache
__device__ __inline__ double load_glob(const double *val) {
double return_value;
