...
// #define B_SH_SIZE 1024

// #define IMB_PWMIN 128
// #define B_PWMIN 64
// #define IMB_MIN 2048
// #define B_MIN 1024
// #define IMB_PW_SH_SIZE 8192
// #define B_PW_SH_SIZE 4096
// #define IMB_SH_SIZE 4096
// #define B_SH_SIZE 2048
...
// #include "../../BCMGX/src/utility/memory.h"
// #include "../../BCMGX/src/utility/setting.h"
...
static inline int env_force_merge() {
    static int cached = -1;
    if (cached != -1) return cached;
    const char *e = getenv("NSPARSE_FORCE_MERGE");
    if (!e) { cached = 0; return cached; }
    cached = (e[0]=='1' || e[0]=='y' || e[0]=='Y') ? 1 : 0;
    return cached;
}

static inline bool should_use_merge_kernel(int bin_size, int pwarp, int bs, int gs,
                                           int avg_nza, int avg_nzb)
{
    if (env_force_merge()) return true;     // override manuale
    if (bin_size < 512) return true;
    if (pwarp <= 4) return true;
    if (gs <= 4 && bs >= 256) return true;
    // simple safe estimate: if average nnz per row too big for hash table
    const int HASH_BUCKET_SIZE = 2048;
    if (avg_nza + avg_nzb > (HASH_BUCKET_SIZE * 3 / 4)) return true;
    return false;
}
...
template<typename T>
T* my_malloc(size_t num) {
    assert(num);
    T* ret = (T*)malloc(sizeof(T) * (num));
    assert(ret);
    return ret;
}

#define MALLOC(T, num) my_malloc<T>(num)

template<typename T>
T* my_cuda_malloc(size_t num, const char *file, int line) {
    assert(num);
    T* ret = NULL;
    cudaError_t err = cudaMalloc(&ret, sizeof(T) * (num));
    if (err != cudaSuccess) {
        fprintf(stderr, "CUDA err at %s:%d: %s\nnum=%zu\n", file, line, cudaGetErrorString(err), num);
        exit(1);
    }
    return ret;
}

#define CUDA_MALLOC(T, num) my_cuda_malloc<T>(num, __FILE__, __LINE__)
...
void init_bin(sfBIN *bin, int M)
{
    ...
    if (global_bin_stream_flag == 0)
        bin->stream = MALLOC(cudaStream_t, BIN_NUM);
    // -------------------------------
    ...
    // -------------------------------------------------------------------------------
    if (global_bin_stream_flag == 0) { 
        bin->bin_size = MALLOC(int, BIN_NUM);
        bin->bin_offset = MALLOC(int, BIN_NUM);
        bin->d_max = CUDA_MALLOC(int, 1);
        bin->d_bin_size = CUDA_MALLOC(int, BIN_NUM);
        bin->d_bin_offset = CUDA_MALLOC(int, BIN_NUM);
        global_bin_stream_flag = 1;
    }
    if (M > global_bin_static_M) {
        if (global_bin_static_M > 0) {
            CUDA_FREE(bin->d_row_perm);
            CUDA_FREE(bin->d_row_nz);
        }
        bin->d_row_perm = CUDA_MALLOC(int, M);
        bin->d_row_nz = CUDA_MALLOC(int, M + 1);
...
void release_bin(sfBIN bin)
{
    ...
    CUDA_FREE(bin.d_row_perm);
    CUDA_FREE(bin.d_max);
    CUDA_FREE(bin.d_bin_size);
    CUDA_FREE(bin.d_bin_offset);
...
void set_max_bin(int *d_arpt, int *d_acol, int *d_brpt, csrlocinfo *binfo, sfBIN *bin, int M)
{
    ...
    LOG_KERNEL(set_intprod_num<<<GS, BS>>>(d_arpt, d_acol, d_brpt, DBINFO, bin->d_row_nz, bin->d_max, M));
    CHECK_CUDA();
    cudaMemcpy(&(bin->max_intprod), bin->d_max, sizeof(int), cudaMemcpyDeviceToHost);
  
    if (bin->max_intprod > IMB_PWMIN) {
        LOG_KERNEL(set_bin<<<GS, BS>>>(bin->d_row_nz, bin->d_bin_size, bin->d_max, M, IMB_MIN, IMB_PWMIN));
        CHECK_CUDA();
        ...
        LOG_KERNEL(set_row_perm<<<GS, BS>>>(bin->d_bin_size, bin->d_bin_offset, bin->d_row_nz, bin->d_row_perm, M, IMB_MIN, IMB_PWMIN));
        CHECK_CUDA();
    }
    else {
        bin->bin_size[0] = M;
        ...
        LOG_KERNEL(init_row_perm<<<GS, BS>>>(bin->d_row_perm, M));
        CHECK_CUDA();
    }
}
...
void set_min_bin(sfBIN *bin, int M)
{
    ...
    BS = 1024;
    GS = div_round_up(M, BS);
    LOG_KERNEL(set_bin<<<GS, BS>>>(bin->d_row_nz, bin->d_bin_size,
                        bin->d_max,
                        M, B_MIN, B_PWMIN));
    CHECK_CUDA();                        
    ...
    if (bin->max_nz > B_PWMIN) {
        ...
        LOG_KERNEL(set_row_perm<<<GS, BS>>>(bin->d_bin_size, bin->d_bin_offset, bin->d_row_nz, bin->d_row_perm, M, B_MIN, B_PWMIN));
        CHECK_CUDA();
    }
...
//     sfBIN bin;
    ...
    // -------------------------------------------------------------------------
    #if DEBUG
    printf("[DEBUG] c->M=%d c->N=%d computed c->nnz=%lld\n", c->M, c->N, (long long)c->nnz);
    if (c->nnz < 0 || c->nnz > (1LL<<40)) {
        fprintf(stderr, "ERROR: suspicious nnz = %lld\n", (long long)c->nnz);
        exit(1);
    }
    #endif
    c->d_col = CUDA_MALLOC(itype, c->nnz);
    c->d_val = CUDA_MALLOC(vtype, c->nnz);
    // -------------------------------------------------------------------------
    ...
    for (i = BIN_NUM - 1; i >= 0; i--) {
        if (bin->bin_size[i] > 0) {
            ...
            case 0:
                ...
                LOG_KERNEL(set_row_nz_bin_pwarp<<<GS, BS, 0, bin->stream[i]>>>
                    (d_arpt, d_acol,
            ...
            case 1 :
				            	...
				            	GS = bin->bin_size[i];
                                CHECK_BS_GS();
            	LOG_KERNEL(set_row_nz_bin_each_tb<512><<<GS, BS, 0, bin->stream[i]>>>
            ...
            case 2 :
				            	...
				            	GS = bin->bin_size[i];
                                CHECK_BS_GS();
            	LOG_KERNEL(set_row_nz_bin_each_tb<1024><<<GS, BS, 0, bin->stream[i]>>>
            	  (d_arpt, d_acol, d_brpt, d_bcol, DBINFO,
            	   bin->d_row_perm, bin->d_row_nz,
            	   bin->bin_offset[i], bin->bin_size[i]));
                CHECK_CUDA();
            ...
            case 3 :
				            	...
				            	GS = bin->bin_size[i];
                                CHECK_BS_GS();
            	LOG_KERNEL(set_row_nz_bin_each_tb<2048><<<GS, BS, 0, bin->stream[i]>>>
            	  (d_arpt, d_acol, d_brpt, d_bcol, DBINFO,
            	   bin->d_row_perm, bin->d_row_nz,
            	   bin->bin_offset[i], bin->bin_size[i]));
                CHECK_CUDA();
            ...
            case 4 :
				            	...
				            	GS = bin->bin_size[i];
                                CHECK_BS_GS();
            	LOG_KERNEL(set_row_nz_bin_each_tb<4096><<<GS, BS, 0, bin->stream[i]>>>
            	  (d_arpt, d_acol, d_brpt, d_bcol, DBINFO,
            	   bin->d_row_perm, bin->d_row_nz,
            	   bin->bin_offset[i], bin->bin_size[i]));
                CHECK_CUDA();
            ...
            case 5 :
				            	...
				            	GS = bin->bin_size[i];
                                CHECK_BS_GS();
            	LOG_KERNEL(set_row_nz_bin_each_tb<8192><<<GS, BS, 0, bin->stream[i]>>>
            	  (d_arpt, d_acol, d_brpt, d_bcol, DBINFO,
            	   bin->d_row_perm, bin->d_row_nz,
            	   bin->bin_offset[i], bin->bin_size[i]));
                CHECK_CUDA();
            ...
            case 6 :
            	    ...
            	    fail_count = 0;
            	    d_fail_count = CUDA_MALLOC(int, 1);
                    d_fail_perm = CUDA_MALLOC(int, bin->bin_size[i]);
            	    cudaMemcpy(d_fail_count, &fail_count, sizeof(int), cudaMemcpyHostToDevice);
            	    BS = 1024;
            	    GS = bin->bin_size[i];
                    CHECK_BS_GS();
            	    LOG_KERNEL(set_row_nz_bin_each_tb_large<8192><<<GS, BS, 0, bin->stream[i]>>>
            	      (d_arpt, d_acol, d_brpt, d_bcol, DBINFO,
            	       bin->d_row_perm, bin->d_row_nz,
            	    ...
            	    if (fail_count > 0) {
            	        ...
            	        int *d_check;
            	        d_check = CUDA_MALLOC(int, table_size);
            	        BS = 1024;
            	        GS = div_round_up(table_size, BS);
            	        LOG_KERNEL(init_check<<<GS, BS, 0, bin->stream[i]>>>(d_check, table_size));
                        CHECK_CUDA();
            	        GS = bin->bin_size[i];
                        CHECK_BS_GS();
	                    LOG_KERNEL(set_row_nz_bin_each_gl<<<GS, BS, 0, bin->stream[i]>>>
                     		  (d_arpt, d_acol, d_brpt, d_bcol, DBINFO,
	            ...
	            CUDA_FREE(d_fail_count);
	            CUDA_FREE(d_fail_perm);
	        }
	        break;
	      default :
	          exit(0);
...
void print_info(int i, int GS, int BS, sfBIN *bin) {
    printf("bin_offset[%d] = %d\n", i, bin->bin_offset[i]);
    printf("bin_size[%d] = %d\n", i, bin->bin_size[i]);
    fflush(stdout);
    int host_row_perm[bin->bin_size[i]*PWARP];
    cudaMemcpy(host_row_perm, bin->d_row_perm + bin->bin_offset[i],
           sizeof(int)*bin->bin_size[i]*PWARP, cudaMemcpyDeviceToHost);
    for(int k=0;k<bin->bin_size[i]*PWARP;k++)
        printf("row_perm[%d]=%d\n", k, host_row_perm[k]);
    fflush(stdout);
}
...
//       GS = div_round_up(bin->bin_size[i] * PWARP, BS);
//       #if DEBUG
//       printf("DBG: bin %d: bin_size=%lld, PWARP=%d, BS=%d, GS=%lld, bin_offset=%lld, stream=%p\n",
//             i, (long long)bin->bin_size[i], PWARP, BS, (long long)GS, (long long)bin->bin_offset[i], (void*)bin->stream[i]);
//       fflush(stdout);
//       print_info(i, GS, BS, bin);
//       #endif
//       CHECK_BS_GS();
//       LOG_KERNEL(calculate_value_col_bin_pwarp<<<GS, BS, 0, bin->stream[i]>>>
...
// 	  GS = bin->bin_size[i];
//       CHECK_BS_GS();
// 	  LOG_KERNEL(calculate_value_col_bin_each_tb<256><<<GS, BS, 0, bin->stream[i]>>>
// 	    (d_arpt, d_acol, d_aval,
// 	     d_brpt, d_bcol, d_bval, DBINFOV,
// 	     d_crpt, d_ccol, d_cval,
// 	     bin->d_row_perm, bin->d_row_nz,
...
// 	  GS = bin->bin_size[i];
//       CHECK_BS_GS();
// 	  LOG_KERNEL(calculate_value_col_bin_each_tb<512><<<GS, BS, 0, bin->stream[i]>>>
// 	    (d_arpt, d_acol, d_aval,
// 	     d_brpt, d_bcol, d_bval, DBINFOV,
// 	     d_crpt, d_ccol, d_cval,
// 	     bin->d_row_perm, bin->d_row_nz,
...
// 	  GS = bin->bin_size[i];
//       CHECK_BS_GS();
// 	  LOG_KERNEL(calculate_value_col_bin_each_tb<1024><<<GS, BS, 0, bin->stream[i]>>>
// 	    (d_arpt, d_acol, d_aval,
// 	     d_brpt, d_bcol, d_bval, DBINFOV,
// 	     d_crpt, d_ccol, d_cval,
// 	     bin->d_row_perm, bin->d_row_nz,
...
// 	  GS = bin->bin_size[i];
//       CHECK_BS_GS();
// 	  LOG_KERNEL(calculate_value_col_bin_each_tb<2048><<<GS, BS, 0, bin->stream[i]>>>
// 	    (d_arpt, d_acol, d_aval,
// 	     d_brpt, d_bcol, d_bval, DBINFOV,
// 	     d_crpt, d_ccol, d_cval,
// 	     bin->d_row_perm, bin->d_row_nz,
...
// 	  GS = bin->bin_size[i];
//       CHECK_BS_GS();
// 	  LOG_KERNEL(calculate_value_col_bin_each_tb<4096><<<GS, BS, 0, bin->stream[i]>>>
// 	    (d_arpt, d_acol, d_aval,
// 	     d_brpt, d_bcol, d_bval, DBINFOV,
// 	     d_crpt, d_ccol, d_cval,
// 	     bin->d_row_perm, bin->d_row_nz,
...
// 	    real *d_value;
// 	    d_check = CUDA_MALLOC(int, table_size);
//         d_value = CUDA_MALLOC(real, table_size);
// 	    BS = 1024;
// 	    GS = div_round_up(table_size, BS);
// 	    LOG_KERNEL(init_check<<<GS, BS, 0, bin->stream[i]>>>(d_check, table_size));
//         CHECK_CUDA();
// 	    LOG_KERNEL(init_value<<<GS, BS, 0, bin->stream[i]>>>(d_value, table_size));
//         CHECK_CUDA();
// 	    GS = bin->bin_size[i];
//         CHECK_BS_GS();
// 	    LOG_KERNEL(calculate_value_col_bin_each_gl<<<GS, BS, 0, bin->stream[i]>>>
// 	      (d_arpt, d_acol, d_aval,
// 	       d_brpt, d_bcol, d_bval, DBINFOV,
// 	       d_crpt, d_ccol, d_cval,
// 	       bin->d_row_perm, bin->d_row_nz,
...
/* =========================
   MODIFIED calculate_value_col_bin  (Version 1 - Uniform fallback)
   =========================
   Behavior:
   - For case 0, instead of launching calculate_value_col_bin_pwarp (shared-hash),
     we allocate global tables (d_check, d_value) sized max_row_nz * bin_size
     and call calculate_value_col_bin_each_gl, a merge-style global kernel
     already present in the file. Other cases are preserved unchanged.
*/
                             ...
                             int M)
{
    ...
    for (i = BIN_NUM - 1; i >= 0; i--) {
        if (bin->bin_size[i] > 0) {
            ...
            case 0:
            {
                // UNIFORM FALLBACK: use global merge kernel for the whole bin
                // to avoid deadlock issues of per-warp shared-hash kernel.

                // Defensive compute of table size
                int max_row_nz = bin->max_nz * 2;
                long long table_size_ll = (long long)max_row_nz * (long long)bin->bin_size[i];
                if (table_size_ll <= 0) table_size_ll = 1;
                size_t table_size = (size_t)table_size_ll;

                int *d_check = NULL;
                real *d_value = NULL;

                // allocate device scratch arrays (int and real)
                d_check = CUDA_MALLOC(int, table_size);
                d_value = CUDA_MALLOC(real, table_size);

                // init scratch arrays
                BS = 1024;
                GS = div_round_up((int)table_size, BS);
                LOG_KERNEL(init_check<<<GS, BS, 0, bin->stream[i]>>>(d_check, table_size));
                CHECK_CUDA();
                LOG_KERNEL(init_value<<<GS, BS, 0, bin->stream[i]>>>(d_value, table_size));
                CHECK_CUDA();

                // launch the safe global kernel (merge style)
                GS = bin->bin_size[i];
                CHECK_BS_GS();
                LOG_KERNEL(calculate_value_col_bin_each_gl<<<GS, BS, 0, bin->stream[i]>>>
                  (d_arpt, d_acol, d_aval,
