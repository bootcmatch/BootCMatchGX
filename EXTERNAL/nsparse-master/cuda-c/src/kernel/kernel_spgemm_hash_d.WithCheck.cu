	...
	SELROW(acol);
	    ...
	    bcol=bacol[k];
			...
			nz++;
                        ...
                        if (check[adr] != key){ //@@@@@ CONTROLLO AGGIUNTO PER EVITARE DUPLICAZIONE
            ...
            if (d_check[adr] != key) {
                  ...
                  while (1){
                     ...
                     if (old == -1) {
                        ...
                        if (d_check[adr] != key){ //@@@@@ CONTROLLO AGGIUNTO PER EVITARE DUPLICAZIONE
...
__global__ void calculate_value_col_bin_pwarp(const int *d_arpt,
	...
	SELVALROW(acol);
	if((brow[acol-rowoff + 1]-brow[acol-rowoff])>B_PWMIN) {
	   printf("Unexpected number of elements: %d\n", (brow[acol-rowoff + 1]-brow[acol-rowoff]));
	   assert(0);
	}
        for (k = brow[acol-rowoff]; k < brow[acol-rowoff + 1]; k++) {
	    bcol=bacol[k];
	        ...
	        if (shared_check[adr] != key) {
                  ...
                  while (1){
                     ...
                     if (old == -1) {
                        ...
                        if (shared_check[adr] != key){ //@@@@@ CONTROLLO AGGIUNTO PER EVITARE DUPLICAZIONE
                           hash = (hash + 1) & (SH_ROW - 1);
                           adr = soffset + hash;
                        } else {
                           atomic_fadd(shared_value + adr, aval * bval);
	    ...
	    if (shared_check[hash] != key) {
                  ...
                  while (1){
                     ...
                     if (old == -1) {
                        ...
                        if (shared_check[hash] != key){ //@@@@@ CONTROLLO AGGIUNTO PER EVITARE DUPLICAZIONE
                           hash = (hash + 1) & (SH_ROW - 1);
                        } else {
                           atomic_fadd(shared_value + hash, aval * bval);
                           break;
...
{
    int myid, nprocs, lnnz; //MPI_Comm_rank(MPI_COMM_WORLD, &myid); MPI_Comm_size(MPI_COMM_WORLD,&nprocs);
    double starttime, smbtime, srnnztime, malltime, calctime, endtime;  
    myid=getpid();
...
//    malltime=MPI_Wtime();
    ...
    fprintf(stderr,"Task %d, entering calculate_value_col_bin\n",myid);
    calculate_value_col_bin(a->d_rpt, a->d_col, a->d_val,
                            b->d_rpt, b->d_col, b->d_val, binfo,
                            c->d_rpt, c->d_col, c->d_val,
                            &bin,
                            M);
    fprintf(stderr,"Task %d, out of calculate_value_col_bin\n",myid);
//    calctime=MPI_Wtime();
//    endtime=MPI_Wtime();
//    if(myid==0) {
//            printf("%d tasks, task %d, time=%f, set_max_bin_time=%f, set_row_nnz_time=%f, malloc_time=%f, calctime=%f M=%d, nnz=%d, bin size=%d %d %d %d %d %d %d\n",nprocs,myid,endtime-starttime,
//          smbtime-starttime,srnnztime-smbtime,malltime-srnnztime,endtime-malltime,M,lnnz,bin.bin_size[6],bin.bin_size[5],bin.bin_size[4],bin.bin_size[3],bin.bin_size[2],bin.bin_size[1],bin.bin_size[0]);
//    }
